...
<article class="comment mt-2" data-id="{{ $post->id }}">
	...
	<div class="flex items-center">
		...
		@if ($owner == false && $post->user && Auth::check())
			<button class="tool-link" onclick="showReportPostModal('comment', {{ $comment->id }}, '{{ $post->text }}')">
		...
		@endif
		@if ($owner || $elevated)
			<button onclick="showDeleteModal({{ $post->id }}, deleteComment, setupDeleteComment)" class="tool-link text-red-500">
